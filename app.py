...
user = int(st.number_input('Enter the user id to whom you want to recommend bewtween 1 to 998 :',min_value=0,max_value=998,step=1))
if user>0:
    
    predicted_policys = User_item_score1(user)
    
    st.markdown('**Policys That are Recommended**')
    if predicted_policys!=[]:
        for i in predicted_policys:
            st.write(i)
    else:
        st.write("No recommended policy")
    st.markdown('**Policys List**')
    st.write(policy_data[['policy_title','category']])
